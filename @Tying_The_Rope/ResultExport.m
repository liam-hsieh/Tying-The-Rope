...
for i=2:size(progress,1)
    % replace product index by product name
    if progress{i,3}==0
        progress{i,3}='';
    else
    ...
    % replace site index by site name
    if progress{i,4}==0
        progress{i,4}='';
    else
...
row_label = string([SL_label ; prod_SL_label; site_exp_label; site_P10_label]);

values = zeros(size(row_label, 1), obj.Pars.num_quarter);
...
values = [row_label string(values)];

sheetname = 'Metrics under Recomended Qual';
column_label = string([{''} {''} obj.BasicInfo.quarter]);
row_label = [];
...
% 
% num_site = size(obj.BasicInfo.site, 2);
% empty_row = string([repmat({' '}, 1, num_site +1)]);
% full_stack = empty_row;
% for q = 1:obj.Pars.num_quarter
%     column_label = string([obj.BasicInfo.quarter(q) obj.BasicInfo.site]);
...
obj.Export2Excel(obj.Qmat.proposed, obj.Dir.output, filename, sheetname, string(row_label), string(column_label));
