...
% Prepare the linear programming (LP) or mixed-integer programming (MIP) problem
...
elseif strcmp(mode, 'MIP') %mode=='MIP'
    margin_matrix = repmat(obj.Demand.margin, [1,m]);
    f_x = reshape(transpose(margin_matrix - obj.Setting.cost_matrix), [],1);
    f_y = zeros(m,1);
    f_u = -ones(m,1) * obj.Setting.wt_minUObj;
...
%% left-hand-side
% Initialize left-hand-side matrices based on LP or MIP mode
if strcmp(mode, 'LP')%mode=='LP'
    %% cons (III-1)
    v1 = 1:n;
...
if strcmp(mode, 'LP')  %mode=='LP'
    lb = zeros(n*m+m, 1);
    ub_x = reshape(transpose(obj.Qmat.(mat_name)), [], 1)*999; % upper bound for x_ij
    ub_u = ones(m, 1)*999;                                     % upper bound for u_j
    ub = vertcat(ub_x, ub_u);
elseif strcmp(mode, 'MIP') %mode=='MIP'
    lb = zeros(n*m +2*m, 1);
    ub_x = reshape(transpose(obj.Qmat.(mat_name)),[], 1)*999;  % upper bound for x_ij
    ub_y = ones(m, 1);                                         % upper bound for y_j
    ub_u = obj.Capacity(:,q);                                  % upper bound for u_j
...
if strcmp(mode, 'LP')  %mode=='LP'
    ...
    solver_options = optimoptions('linprog', 'algorithm', 'interior-point', 'Display', 'off'); %the default algorithm is'dual-simplex' if not set aside
